...
character(*),parameter::path='/glade/scratch/liqi1026/0307_LES512/'
...
integer,parameter::nx=1024,ny=512,nz=(640-1)/nproc + 1
...
integer,parameter::nsteps=6000 ! simulation steps + SCAL_init for 32^3 with dt_dim=0.25
...
real(rprec),parameter::dt_dim=0.04_rprec ! this is for unstable wt_s = 0.1, 64 cube case
real(rprec)::dt=dt_dim*u_star/z_i
! real(rprec),parameter::dt=dt_dim*u_star/z_i

real(rprec),parameter::coriol=1.3942923E-04*z_i/u_star !for latitude = 37.6N ? or 73N ?
real(rprec),parameter::ug=ug_dim/u_star,vg=vg_dim/u_star

real(rprec),parameter::vonk=.4_rprec

! SKS
integer,parameter::c_count=100,p_count=100!p_count=800 => 5 mins for 64^3;dt=0.003
...
integer, parameter :: average_dim_num = 2
