...
real(kind=rprec),dimension(ld,ny)::surf_temp_pr_modi
! remove this stuff!
integer::jx,jy,jz,k
...
if ((.not. USE_MPI) .or. (USE_MPI .and. coord == 0)) then
!      if ((jt .gt. SCAL_init) .and. (remote_flag .eq. 1)) then
      if ((jt .gt. SCAL_init) .and. (lbc .eq. 0)) then
!     Pressure bc modification at the surface for surf temp bc
      surf_temp_pr_modi=0._rprec
      surf_temp_pr_modi(1:nx,:)=g*(z_i/(u_star**2))*(T_s-sum(T_s)/real(nx*ny))
      if (jt .eq. nsteps) print *,'surf pr correction = ',surf_temp_pr_modi(2:3,2:3)
