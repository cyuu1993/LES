...
if ( read_inflow_file ) then
   FF_LM(1,2:ny+1,2:nz+1)=FLM_hold
   FF_LM(nx+2,:,:) = FF_LM(nx+1,:,:)
else
   FF_LM(1,:,:) = FF_LM(nx+1,:,:)
   FF_LM(nx+2,:,:) = FF_LM(2,:,:)
end if
...
if (read_inflow_file) then
  FF_MM(1,2:ny+1,2:nz+1) = FMM_hold
  FF_MM(nx+2,:,:) = FF_MM(nx+1,:,:) 
else
  FF_MM(1,:,:) = FF_MM(nx+1,:,:)
  FF_MM(nx+2,:,:) = FF_MM(2,:,:)
end if
...
if (read_inflow_file) then
  FF_QN(1,2:ny+1,2:nz+1)= FQN_hold
  FF_QN(nx+2,:,:) = FF_QN(nx+1,:,:)
else
  FF_QN(1,:,:) = FF_QN(nx+1,:,:)
  FF_QN(nx+2,:,:) = FF_QN(2,:,:)
end if
...
if ( read_inflow_file ) then
  FF_NN(1,2:ny+1,2:nz+1) = FNN_hold
  FF_NN(nx+2,:,:) = FF_NN(nx+1,:,:) 
else
  FF_NN(1,:,:) = FF_NN(nx+1,:,:)
  FF_NN(nx+2,:,:) = FF_NN(2,:,:)
end if
